...
def test_edit_article_callback_permission(app, app_client, user_info, expected_status_code, edit_workflow):
    if user_info:
        login_user_via_session(app_client, email=user_info['email'])
    else:
    ...
    control_number = edit_workflow.data['control_number']
    endpoint_url = "/callbacks/resolve_edit_article/{}".format(control_number)
    data_payload = {
    ...
    response = app_client.post(endpoint_url, data=json.dumps(data_payload))
    assert response.status_code == expected_status_code
